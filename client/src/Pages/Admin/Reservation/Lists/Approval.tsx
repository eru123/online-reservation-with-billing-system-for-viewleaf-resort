...
function Approval() {
  ...
  useEffect(()=>{
    if(reservations!= null && reservations != undefined){
      const Filtered = reservations.filter((reservation:any) => {
        return (
          ['paid'].includes(reservation.status) 
        )
      })
  ...
  return (
    <div>
      <Typography variant="h4" fontWeight={600} color="primary">Approval List</Typography>
      <Typography variant="h6" fontWeight={400} color="initial" sx={{marginBottom:"2em"}}>Here are all the list of to be approved</Typography>
